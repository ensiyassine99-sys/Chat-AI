        ...
        <div className={`flex-1 min-w-0 ${isUser ? (isRTL ? 'items-start' : 'items-end') : (isRTL ? 'items-end' : 'items-start')} flex flex-col`}>
          <div
            className={`px-5 py-3 rounded-2xl shadow-lg ${isUser
                ? `bg-gradient-to-r from-blue-600 to-purple-600 text-white ${isRTL ? 'rounded-tl-sm' : 'rounded-tr-sm'}`
                : `bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 ${isRTL ? 'rounded-tr-sm' : 'rounded-tl-sm'}`
              }`}
          >
            {isEditingThis ? (
              <div className="space-y-3">
                <textarea
                  ...
                  className={`w-full p-3 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none ${isRTL ? 'text-right' : 'text-left'
                    }`}
                  rows={4}
                  autoFocus
                />
                <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={saveEdit}
                    className="px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl hover:from-green-700 hover:to-green-800 transition-all shadow-lg flex items-center gap-2"
                  >
              ...
              <div className={`prose max-w-none ${isUser ? 'prose-invert' : 'prose-slate dark:prose-invert'}`}>
                {isUser ? (
                  <p className={`whitespace-pre-wrap m-0 text-sm leading-relaxed ${isRTL ? 'text-right' : 'text-left'}`}>
                    {message.content}
                  </p>
                ) : (
                  <>
                        ...
                        components={{
                          code({ node, inline, className, children, ...props }) {
                            ...
                            return !inline && match ? (
                              ...
                              <code
                                className="bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-lg text-sm font-mono"
                                {...props}
                              >
                                {children}
                              </code>
